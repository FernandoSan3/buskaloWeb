 ...
 public function show($user_id)
    {
        ...
        // ->leftjoin('social_networks','users.id','=','social_networks.user_id')
        ->select('users.*')
        ->where('users.id',$user_id)->first();

        return view('backend.client.view_company',compact('company_details'));
    }

    public function ratings_reviews ($user_id)
    {
        $userId = $user_id;
        ...
        return view('backend.company.ratings_reviews',compact('ratings_reviews'));
    }
    ...
    public function edit_ratings_reviews($user_id)
    {
        ...
        return view('backend.company.edit_ratings_reviews', compact('ratings_reviews','total_service_requests'));
    }

    public function update_ratings_reviews(Request $request)
    {
        $ratings_reviews_id = $request->ratings_reviews_id;
        ...
        ->leftjoin('users', 'reviews.to_user_id', '=', 'users.id')
        ->select('reviews.*','users.username')
        ->where('reviews.id', $ratings_reviews_id)
        ->where('reviews.deleted_at', NULL)->first();
        ...
        return redirect()->route('admin.company.ratings_reviews', ['id' => $ratings_reviews->to_user_id])->with('success','Rating & Review updated successfully.');
    }
    ...
    public function destroy_ratings_reviews($reviews_id)
    {
        ...
        ->leftjoin('users', 'reviews.to_user_id', '=', 'users.id')
        ->select('reviews.*','users.username')
        ->where('reviews.id', $reviews_id)
        ->where('reviews.deleted_at', NULL)->first();

        DB::table('reviews')->where('id', $reviews_id)->update($update_arr);

        return redirect()->route('admin.company.ratings_reviews', ['id' =>  $ratings_reviews->user_id])->with('success','Rating & Review deleted successfully.');
    }
    ...
    public function edit($user_id)
    {
        ...
        $user_group_details = [];
        $user_group_details = [ 'id' => 1, 'name' => 'ADMIN' ];
        $user_group_details = [ 'id' => 2, 'name' => 'CLIENTE' ];
        $user_group_details = [ 'id' => 3, 'name' => 'PROFESIONAL' ];
        $user_group_details = [ 'id' => 4, 'name' => 'EMPRESA' ];
        // $user_group_details = [1,2,3,4];
        
        // print_r($user_group_details);
        return view('backend.client.edit',compact('user_details', 'user_group_details'));
    }
    ...
    public function update(Request $request)
    {
       ...
       $user_details = DB::table('users')
        ...
        $update_arr['user_group_id'] = $request->user_group_id;
        ...
        DB::table('users')->where('id',$request->user_id)->update($update_arr);
   ...
   // echo "<pre>"; print_r($combinedData); die('con');
    ...
    public function destroy($user_id)
    {
        ...
        return redirect()->route('admin.client.deleted')->with('success','Cliente deleted successfully');
    }
    ...
    public function allRequestsByStatus($status,$user_id)
    {
     ...
     $users = DB::table('users')->latest()->whereNotNull('deleted_at')
             ->where('user_group_id',2)->paginate(25);

        return view('backend.client.deleted',compact('users'));
    }
     ...
     public function restore($user_id) {
        ...
        DB::table('users')->where('id',$user_id)->update($restore_arr);

       return redirect()->route('admin.client.index')->with('success','Client restored successfully.');
   }
