      ...
      public function index()
        {
            ...
            $userid= auth()->user()->id;
              ...
              if(!empty($userid))
                                 ...
                                 if(!empty($servicesbuy))
                                   {
                                    ...
                                    foreach ($servicesbuy as $key => $vall)
                                    {
                                        ...
                                        array_push($allData, $data1);
			         					...
			         					if(!empty($allData))
			                            {
			                               return view('frontend.company.jobs')->withData($allData);
			                            }
			                            else
			                            {
			                               return redirect()->route('frontend.company.company_profile')->withFlashDanger(__('job List Not Found.!'));
			                            }
                    ...
                    {
                        return redirect()->route('frontend.company.company_profile')->withFlashDanger(__('Invalid parameter'));
                    }
              ...
              if(!empty($userid) && !empty($job_id))
                    {
                            ...
                            if(!empty($userEntity))
                            {
	                            ...
	                            {
	                                return redirect()->route('frontend.company.company_profile')->withFlashDanger(__('List not found.!'));
	                            }

                            }
                            else
                            {
                               return redirect()->route('frontend.contractor.co-dashoard')->withFlashDanger(__('Invalid user.!'));
                            }
                    }else
                    {
                      return redirect()->route('frontend.contractor.co-dashoard')->withFlashDanger(__('Invalid parameter.!'));
                    }
                    ...
                    $servicesOffered=DB::table('services_offered')->select('service_id')->where('user_id',$userEntity->id)->whereRaw("(deleted_at IS null )")->get()->toArray();
                        ...
                        if(!empty($servicesOffered))
                        {
                           ...
                           if(!empty($allOpprtunities))
                           {
                            ...
                            if(!empty($data1))
                            {
                                return view('frontend.company.opportunity')->withData($allData);
                            }
                            else
                            {
                                return redirect()->route('frontend.company.company_profile')->withFlashDanger(__('Opportunities List Not Found.!'));
                            }

                           }
                           else
                           {
                            return redirect()->route('frontend.company.company_profile')->withFlashDanger(__('Opportunities Not Found.!'));
                           }
                        ...
                        else
                        {
                           ...
                           return redirect()->route('frontend.company.company_profile')->withFlashDanger(__('Please update your profile for your offerd services.'));
                        }
                        ...
                        ->first();
                          ...
                          if($opportunity)
                          {
                              if($opportunity->status!=='0')
                              {
                                  ...
                                  return redirect()->route('frontend.company.company_profile.opportunities')->withFlashDanger(__('Requested opportunity is already Assigned another professionals.'));
                              ...
                              }else
                              {
                                  ...
                                  if(!empty($chkUserRecivedOpprtOrNot))
                                   {
                                      ...
                                      if($chkUserRecivedOpprtOrNot->request_status=='buy')
                                      {
                                        ...
                                        return redirect()->route('frontend.company.opportunities')->withFlashDanger(__('Opportunity Already Accepted.!'));
                                      ...
                                      } else
                                       {
                                          ...
                                          if($chkThreeUserLimit < 3)
                                          {
                                              ...
                                              ->whereRaw("(id = '".$opportunity_id."' AND user_id = '".$userid."')")->update($update_Arr);
                                               ...
                                               return redirect()->route('frontend.contractor.company_profile.opportunities')->withFlashDanger(__('Opportunity Buy Successfully.!'));

                                           }else
                                           {
                                              return redirect()->route('frontend.company.company_profile.opportunities')->withFlashDanger(__('This Opportunity Alfready Taken By Another Three professionals OR Company.!'));
                                           }
                                      }
                                 ...
                                 {
                                           return redirect()->route('frontend.company.company_profile.opportunities')->withFlashDanger(__("You don't have this opportunity.Please update your profile offerd services to get new Opportunities."));
                                 }
                          ...
                          {
                              return redirect()->route('frontend.company.company_profile.opportunities')->withFlashDanger(__("Requested opportunity id not found.!"));
                          }

                      }
                      else
                      {
                          return redirect()->route('frontend.company.company_profile.opportunities')->withFlashDanger(__("Invalid user.!"));
                      }

              }
              else
              {
              	return redirect()->route('frontend.company.company_profile.opportunities')->withFlashDanger(__("Invalid parameter.!"));
              }
