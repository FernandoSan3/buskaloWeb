...
class MessagesController extends Controller
{
    ...
    public function getLoadLatestMessages(Request $request)
    {
        ...
        foreach ($messages as $message) 
        {
            ...
            //$return[] = view('frontend.chat-line')->with('message', $message)->render();

         if(Auth::user()->role_id==4)
         {
            $return[] = view('frontend.program-manager.chat-line')->with('message', $message)->render();
         }
         else if(Auth::user()->role_id==6)
         {
            $return[] = view('frontend.mentor.chat-line')->with('message', $message)->render();
         }
