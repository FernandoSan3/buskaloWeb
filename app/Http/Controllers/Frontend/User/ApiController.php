            ...
            else
            {
                ...
                }else
                {
                      ...
                      if(($logincount==0) && ($users_count_var->user_group_id==3 || $users_count_var->user_group_id==4))
                    {
                        ...
                        $freecredit=DB::table('site_settings')->where('id',1)->first();
                        $useradd=isset($freecredit->free_credit)?$freecredit->free_credit:'20';
                        $message=trans('apimessage.Your profile is activated successfully, and you have recived bonus of'. $useradd.' credits.!');
                         DB::table('user_devices')->where('user_id',$users_count_var->id)->update(['login_count'=>1]);
                    }
                    else
                    {
    ...
    /* ------------------------------------------------------------------------------------------- */
         ...
         public function sendApplicationRequest(Request $request)
         {
                ...
                $queOptionData = json_decode(json_encode($variable_fields_data), true);
                    ...
                    if($validator->fails())
                    {
                        ...
                        echo json_encode($resultArray); exit;      
                            ...
                            // {
                            //     $resultArray['status']='0'; 
                            //     $resultArray['message']=trans('Question type invalid.');
                            //      echo json_encode($resultArray);exit;
                            // }
                ...
                $resultArray['status']='1';  
                 ...
                 $resultArray['service_amount']=$service_credit;
                    ...
                    else if($userEntity->user_group_id==3 || $userEntity->user_group_id==4)
                    {
                        ...
                        if($userEntity->user_group_id==4)
                        {
                           ...
                           if(!empty($users_count_var->username) && !empty($users_count_var->ruc_no) && !empty($users_count_var->legal_representative) && !empty($users_count_var->mobile_number))
                            ...
                            //print_r($total_rating_count); die;
...
/* ------------------------------------------------------------------------------------------------ */
                                    ...
                                    else
                                    {
                                        ...
                                        if($chkThreeUserLimit < 3)
                                        {
                                            ...
                                            {   
                                                ...
                                                if($userEntity->user_group_id==3)
                                                {
                                                    $userEntity->link=url('mi-perfil');
                                                }else
                                                {
                                                   $userEntity->link=url('company_profile/mi-perfil'); 
                                                }
                           ...
                           //->where('assign_service_request.request_status','buy')
                            ...
                            if(!empty($servicesbuy))
                            {
                                ...
                                foreach ($servicesbuy as $key => $vall) 
                                {
                                    ...
                                    // if($vall->hire_status == 0)
                                    // {
                                    //     $servicestatus='new';
                                    // }elseif($vall->hire_status == 1)
                                    // {
                                    //     $servicestatus='Accepted';
                                    // }elseif($vall->hire_status == 2)
                                    // {
                                    //     $servicestatus='Pending';
                                    // }elseif($vall->hire_status == 3)
                                    // {
                                    //     $servicestatus='rejected';
                                    // }
                                    // elseif($vall->hire_status == 4)
                                    // {
                                    //     $servicestatus='service performed';
                                    // }

                                    //$data1['service_status'] = $servicestatus;
                                    $data1['city_name'] = isset($vall->name)?$vall->name:'';
        ...
        $lang = !empty($request->lang) ? $request->lang : 'en' ;
         App::setLocale($lang);
            ...
            if($validator->fails())
            {
                ...
                if(!empty($userid) && !empty($session_key)  && !empty($folderid)) 
                {
                    $check_auth = $this->checkToken($access_token, $userid, $session_key, $lang);
                    if($check_auth['status']!=1)
                    {
    ...
    public function readMessage(Request $request)
    {
        ...
        if($validator->fails())
        {
            ...
            if(!empty($userid) && !empty($from_user_id) && !empty($session_key)) 
            {
    ...
    public function HireProOrCompany(Request $request)
    {
        ...
        if($validator->fails())
        {
            ...
            if(!empty($user_id) && !empty($pro_user_id) && !empty($session_key)) 
            {
        ...
        App::setLocale($lang); 
          $validator = Validator::make($request->all(), [
            ...
            if($validator->fails())
            {
                ...
                if(!empty($user_id) && !empty($client_user_id) && !empty($session_key) && !empty($request_id)) 
                {
                    $check_auth = $this->checkToken($access_token, $user_id, $session_key, $lang);
        ...
        App::setLocale($lang); 
          $validator = Validator::make($request->all(), [
            ...
            if($validator->fails())
            {
                ...
                if(!empty($user_id) && !empty($session_key) && !empty($request_id)) 
                {
                    $check_auth = $this->checkToken($access_token, $user_id, $session_key, $lang);
          ...
          $validator = Validator::make($request->all(), [
            ...
            if($validator->fails())
            {
                ...
                else
                {
                    ...
                    if(!empty($userEntity))
                    {
                        $url= url('payments/'.$subscription_id.'/Profesional/'.$user_id.'?session='.$session_key);
                        $resultArray['status']='1';
                            ...
                            if(empty($prousercheck))
                            {
                                ...
                                // }
                                  ...
                                  Mail::send('frontend.mail.payment_request',  ['data' => $data], function($message) use ($user, $profesionName){
                                    ...
                                    if(empty($prousercheck))
                                    {
                                        DB::table('user_payment_history')->insertGetId($paymentinsert);
                                    }
                                    else
                                    {
    ...
    public function updateSql(Request $request)
    {
        ...
        $getdata= DB::table('assign_service_request')->where('user_id','803')->get();
        $getdata1= DB::table('users')->get();
