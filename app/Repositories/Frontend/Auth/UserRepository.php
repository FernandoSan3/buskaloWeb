...
class UserRepository extends BaseRepository
{
    ...
    public function create(array $data)
    {
        ...
        return DB::transaction(function () use ($data) {
            $user = $this->model::create([
                ...
                'confirmed' => ! (config('access.users.requires_approval') || config('access.users.confirm_otp')),
            ]);
    ...
    public function findOrCreateProvider($data, $provider)
    {
        ...
        $usergroup=User::where('email',$user_email)->first();

        if(!empty($usergroup))
        {
            if($usergroup->email==$user_email && $usergroup->user_group_id!=$user_group_id)
            {
                throw new GeneralException(__('exceptions.frontend.auth.email_taken'));
            }
        }
        ...
        if (! $user) {
            ...
            $user = $this->model::create([
                ...
                'email' => $user_email,
                'user_group_id' =>isset($user_group_id)?$user_group_id:'2',
                'active' => true,
                'confirmed' => true,
                'password' => null,
        ...
        if (! $user->hasProvider($provider)) {
            ...
            $user->user_group_id =isset($user_group_id)?$user_group_id:'2';
            $user->update();
        }
