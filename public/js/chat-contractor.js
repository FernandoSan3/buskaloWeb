...
function loadLatestMessages(container, user_id)
{
    ...
    $.ajax({
        url: base_url + "/contr/load-latest-messages",
...
function fetchOldMessages(to_user, old_message_id)
{
    ...
    let chat_area = chat_box.find(".chat-area");
    $.ajax({
        url: base_url + "/contr/fetch-old-messages",
...
{
    return `
           <div class="row msg_container base_receive" data-message-id="${message.data.id}">
           <div class="col-md-2 col-xs-2 avatar">
