 $(function () {
   ...
   $(".chat-toggle").on("click", function (e) {
      ...
      //First Close old then new
      //$(".close-chat").parents("div.chat-opened").removeClass("chat-opened").slideUp("fast");
   ...
   $(".close-chat").on("click", function (e) {
    ...
    {
       alert("inMinimize");
        $('.addMinimumOrMaximum').removeClass("minimize-chat");
        $('.addMinimumOrMaximum').addClass("maximize-chat");
        $(this).parents('.chat_box').animate({
                height: '40px',
    ...
    $(".maximize-chat").on("click", function (e) 
    {

       alert("inMaximize");
        $('.addMinimumOrMaximum').removeClass("maximize-chat");
        $('.addMinimumOrMaximum').addClass("minimize-chat");
...
function loadLatestMessages(container, user_id)
{
    ...
    $.ajax({
        url: base_url + "/usr/load-latest-messages",
...
{
    return `
           <div class="row msg_container base_sent" data-message-id="${message.data.id}">
        <div class="col-md-10 col-xs-10">
            <div class="messages msg_sent text-right">
                <p>${message.data.content}</p>
                <time datetime="${message.data.dateTimeStr}"> ${message.data.dateHumanReadable} </time>
            </div>
        </div>
        <div class="col-md-2 col-xs-2 avatar">
            <img src="` + base_url +  '/img/frontend/dummy_user.jpg' + `" width="50" height="50" class="img-responsive">
        </div>
    </div>
    `;
}
...
function getMessageReceiverHtml(message)
{
    return `
           ...
           <div class="col-md-2 col-xs-2 avatar">
             <img src="base_url/img/${message.from_user_profile}" width="50" height="50" class="img-responsive">
           </div>
        <div class="col-md-10 col-xs-10">
...
function displayMessage(message)
{
    ...
    if($("#current_user").val() == message.data.from_user_id) {

        let messageLine = getMessageSenderHtml(message);

        $("#chat_box_" + message.data.to_user_id).find(".chat-area").append(messageLine);

    } else if($("#current_user").val() == message.data.to_user_id) {

        alert_sound.play();

        // for the receiver user check if the chat box is already opened otherwise open it
        cloneChatBox(message.data.from_user_id, message.data.fromUserName, function () {

            let chatBox = $("#chat_box_" + message.data.from_user_id);

            if(!chatBox.hasClass("chat-opened")) {

                chatBox.addClass("chat-opened").slideDown("fast");

                loadLatestMessages(chatBox, message.data.from_user_id);
                ...
                $("#chat_box_" + message.data.from_user_id).find(".chat-area").append(messageLine);
            }
        });
    }
}
