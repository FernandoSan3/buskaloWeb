       ...
       <div style="display: inline-block; width: 100%; margin-top: 30px;">
          <h4 style="font-size: 16px; color: #808080; font-weight: 600; margin: 0 0 20px;">
            ...
            {{isset($userToken->message)?$userToken->message:''}} 
             Perfil del usuario:<img style="width: 80px; height: 80px; border-radius: 50%;" src="{{isset($userToken->user_icon)?$userToken->user_icon:url('img/logo/logo.jpg')}}">
                <P>
                <h5>Nombre de usuario: {{$userToken->clientName}}</h5>
                <!-- <h5>Address:{{isset($userToken->addressdata)?$userToken->addressdata:''}}</h5> -->
                <h5>Número de teléfono: {{isset($userToken->mobile_numberdata)?$userToken->mobile_numberdata:''}}</h5>
                <h5>Dirección: {{isset($userToken->addressdata)?$userToken->addressdata:''}}</h5>
                <h5>Correo electrónico: {{isset($userToken->useremail)?$userToken->useremail:''}}</h5>
              </P>
