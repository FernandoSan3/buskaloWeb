    ...
    <div style="background: #f5f5f5; display: inline-block; width: 100%; padding: 10px 0; text-align: center;">
      ...
      <div style="display: block; width: 70%; margin: 50px auto;">
        ...
        <div style="display: inline-block; width: 100%; margin-top: 30px;">
         ...
         <!--  <a href="{{ $data['actionurl']}}"><button style="background: rgb(237,82,22);background: linear-gradient(335deg, rgba(237,82,22,1) 0%, rgba(246,149,105,1) 100%); display: block; margin: 0 auto; min-width: 180px; border: solid 1px #e84621; box-shadow: none; height: 50px; border-radius: 40px; color: #fff; font-size: 16px; cursor: pointer;" >
             {{isset($data['buttontext'])?$data['buttontext']:'Review Rating'}}
            </button>
           <h3><a href="{{$data['actionurl']}}">{{ $data['actionurl']}}</a></h3>
        </a> -->
                <table width="700px">
                    <tr>
                        <th><div><img width="40px" height="40px" src="{{isset($data['profephoto'])?$data['profephoto']:$data['user_icon']}}"></div></th>
                    </tr>
                    <tr>
                        <th>{{isset($data['profename'])?$data['profename']:''}}</th>
                    </tr>
                    <tr>
                        <th> {{isset($data['servicename'])?$data['servicename']:''}}</th>
                    </tr>
                </table>
                <a href="{{ $data['actionurl']}}" style="text-decoration: none;"><button style="background: rgb(237,82,22); display: block; margin: 0 auto; min-width: 180px; border: solid 1px #e84621; box-shadow: none; height: 50px; border-radius: 40px; color: #fff; font-size: 16px; cursor: pointer;" >
                     {{isset($data['buttontext'])?$data['buttontext']:'Review Rating'}}
                    </button>
                   <!-- <h3><a href="{{$data['actionurl']}}">{{ $data['actionurl']}}</a></h3> -->
                </a>
          ...
          <div style="display: inline-block;width: 100%; margin-top: 10px;">
            <p style="font-size: 14px; color: #808080;"> Si el botón no funciona copie y pegue este <a href="{{$data['actionurl']}}"> link <a/> en su URL,si el problema persiste por favor comuníquese con nosotros a
